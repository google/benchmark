...
BenchmarkReporter::BenchmarkReporter()
    : first_report_(true), output_stream_(&std::cout), error_stream_(&std::cerr)
{}
...
void BenchmarkReporter::ReportRuns(std::vector<Run> const& reports) {
  if (reports.empty()) {
    return;
  }
  std::string indent(4, ' ');
...
void BenchmarkReporter::PrintRunData(Run const& run) {
  std::string indent(6, ' ');
  std::ostream& out = GetOutputStream();
