...
typedef uint64_t IterationT;

// BigOFunc is passed to a benchmark in order to specify the asymptotic
// computational complexity for the benchmark.
typedef double(BigOFunc)(IterationT);
...
class State {
 public:
  ...
  //   }
  bool KeepRunningBatch(IterationT n);
 ...
 private
  ...
  IterationT total_iterations_;

  // When using KeepRunningBatch(), batch_leftover_ holds the number of
  // iterations beyond max_iters that were run. Used to track
  // completed_iterations_ accurately.
  IterationT batch_leftover_;

 public:
  const IterationT max_iterations;
 ...
 private:
  State(IterationT max_iters, const std::vector<int64_t>& ranges, int thread_i,
  ...
  bool KeepRunningInternal(IterationT n, bool is_batch);
...
inline BENCHMARK_ALWAYS_INLINE bool State::KeepRunningBatch(IterationT n) {
  return KeepRunningInternal(n, /*is_batch=*/true);
}

inline BENCHMARK_ALWAYS_INLINE bool State::KeepRunningInternal(IterationT n,
                                                               bool is_batch) {
...
class Benchmark {
 public:
  ...
  Benchmark* Iterations(IterationT n);
