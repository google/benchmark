...
import sys
import gbench_cycles
from gbench_cycles import util, report
from gbench_cycles.util import *
...
def main():
    ...
    json1 = json1_orig = gbench_cycles.util.run_or_load_benchmark(
        test_baseline, benchmark_options + options_baseline)
    json2 = json2_orig = gbench_cycles.util.run_or_load_benchmark(
        test_contender, benchmark_options + options_contender)

    # Now, filter the benchmarks so that the difference report can work
    if filter_baseline and filter_contender:
        replacement = '[%s vs. %s]' % (filter_baseline, filter_contender)
        json1 = gbench_cycles.report.filter_benchmark(
            json1_orig, filter_baseline, replacement)
        json2 = gbench_cycles.report.filter_benchmark(
            json2_orig, filter_contender, replacement)

    # Diff and output
    output_lines = gbench_cycles.report.generate_difference_report(
