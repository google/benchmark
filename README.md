...
Additionally the `--benchmark_report_repetitions={true|false}` flag or
`ReportRepetitions(bool)` function can be used to change how repeated tests are
reported. By default the result of each repeated run is reported.
When this option is 'false' only the mean and standard deviation of the runs is
reported. Calling `ReportRepetitions(bool)` on a registered benchmark object
overrides the value of the flag for that benchmark.
